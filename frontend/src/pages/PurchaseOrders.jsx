...
const PurchaseOrders = () => {
    ...
    const submitOrder = async () => {
        try {
            if (editingOrder) {
                await API.put(`/api/purchase-orders/${editingOrder.id}/status`, orderForm);
            } else {
                const apiUrl = orderForm.quotationId ? "/api/purchase-orders/create" : "/api/purchase-orders/create-new";
    ...
    const deleteOrder = async (id) => {
        ...
        try {
            await API.delete(`/api/purchase-orders/delete/${id}`);
            fetchOrders();
    ...
    return (
        <div className="bg-gray-800 p-6 rounded-lg shadow-md">
            <h2 className="text-3xl font-semibold text-white mb-4">Purchase Orders</h2>
            ...
            <table className="min-w-full border-collapse border border-gray-700 mt-6 text-white">
                <thead>
                    <tr className="bg-gray-900">
                        <th className="border border-gray-700 p-3">Seller</th>
                ...
                <tbody>
                    {orders.map((order) => (
                        <tr key={order.id} className="border">
                            <td className="text-center border border-gray-700 p-3">{order.sellerName}</td>
            ...
            {showOrderDetails && selectedOrder && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
                    <div className="bg-gray-900 p-6 rounded-lg shadow-lg w-1/2">
                        ...
                        <p className="text-white"><strong>Seller:</strong> {selectedOrder.sellerName}</p>
                        ...
                        <ul className="text-white">
                            {selectedOrder.OrderItems?.map((item, index) => (
                                <li key={index} className="mt-2">
                                    {item.itemCode} - {item.quantity} x {formatToINR(item.price)}
                                </li>
                            ))}
                        </ul>
